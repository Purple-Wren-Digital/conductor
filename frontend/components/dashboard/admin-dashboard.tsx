...
import { Building, Ticket, Users, TrendingUp, AlertCircle, BarChart } from "lucide-react";
...
export function AdminDashboard() {
  ...
  const [selectedMarketCenter, setSelectedMarketCenter] = useState<string>("all");
  ...
  const filteredTickets = selectedMarketCenter === "all" 
    ? tickets 
  ...
  const filteredUsers = selectedMarketCenter === "all"
    ? allUsers
    : allUsers.filter((u: any) => u.marketCenterId === selectedMarketCenter);
  
  const stats = {
    totalTickets: filteredTickets.length,
    openTickets: filteredTickets.filter((t: any) => t.status !== "RESOLVED").length,
    highPriority: filteredTickets.filter((t: any) => t.urgency === "HIGH" && t.status !== "RESOLVED").length,
    totalUsers: filteredUsers.length,
    agents: filteredUsers.filter((u: any) => u.role === "AGENT").length,
    staff: filteredUsers.filter((u: any) => u.role === "STAFF").length,
    avgResponseTime: "2.4 hours",
    resolutionRate: Math.round((filteredTickets.filter((t: any) => t.status === "RESOLVED").length / filteredTickets.length) * 100) || 0,
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2 items-center">
          <Select value={selectedMarketCenter} onValueChange={setSelectedMarketCenter}>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <CardContent>
            <div className="space-y-4">
              {marketCenters?.marketCenters?.slice(0, 5).map((mc: any) => {
                ...
                const mcOpen = mcTickets.filter((t: any) => t.status !== "RESOLVED").length;
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Latest tickets across all teams
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold">{marketCenters?.marketCenters?.length || 0}</span>
