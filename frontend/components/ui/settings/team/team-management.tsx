...
export default function TeamManagement() {
  ...
  // const formatUserUpdatesBody = (updates: UpdateUserForm) => {
  //   if (!updates || !editingTeamMember) {
  //     console.log("nothing to format");
  //     return {};
  //   }
  //   let body: any = {};
  //   if (userNameDifferent && updates?.name) {
  //     body = { ...body, name: updates.name };
  //   }
  //   if (userEmailDifferent && updates?.email) {
  //     body = { ...body, email: updates.email };
  //   }
  //   if (userRoleDifferent && updates?.role) {
  //     body = { ...body, role: updates.role };
  //   }
  //   if (
  //     userMarketCenterDifferent &&
  //     updates?.marketCenter &&
  //     updates?.marketCenter?.id
  //   ) {
  //     body = {
  //       ...body,
  //       marketCenter: updates.marketCenter,
  //       marketCenterId: updates.marketCenter.id,
  //     };
  //   }
  //   return body;
  // };
  ...
  //   //  TODO: verify staff is editing own market center
  ...
  // const updateAuth0User = async (updates: any) => {
  //   // TODO: authorization for staff
  //   try {
  ...
  //       body: JSON.stringify({
  //         ...updates,
  //         user_id: editingTeamMember?.auth0Id,
  //       }),
  //     });
  //     console.log("updateAuth0User() Response - ", response);
  //     if (!response.ok) {
  //       throw new Error(
  //         response?.statusText
  //           ? response.statusText
  //           : "Failed to update user in Auth0"
  //       );
  //     }
  //     return true;
  //   } catch (error) {
  //     console.error("Failed to update user in Auth0:", error);
  //     return false;
  //   }
  // };
  // const updateTeamMember = async (updates: any) => {
  //   // TODO: authorization for staff

  //   try {
  //     const accessToken = getAuth0AccessToken();
  //     const response = await fetch(
  //       `${API_BASE}/users/${editingTeamMember?.id}/update`,
  //       {
  //         method: "PATCH",
  //         headers: {
  //           "Content-Type": "application/json",
  ...
  //   mutationFn: async (user: PrismaUser) => {
  //     if (!user?.id || !user?.auth0Id) throw new Error("Missing user ids");
  //     console.log("role?", currentUser?.role);

  //     const body = formatUserUpdatesBody(formData);
  //     const updateAuth0Response = await updateAuth0User(body);
  //     // const updateDBResponse = await updateTeamMember(body);
  //     if (!updateAuth0Response)
  //       // || !updateDBResponse)
  ...
  const handleSubmitUserUpdate = async (e: React.FormEvent) => {
    ...
    // updateUserMutation.mutate(editingTeamMember);
    setIsSubmitting(false);
